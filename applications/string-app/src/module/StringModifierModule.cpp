...
defineInputValue(InputHarmNM, "harmNM");
defineInputValue(InputOutputIntermediate, "outputIntermediate");
defineInputValue(InputOutputIntermediateStart, "outputIntermediateStart");

void StringModifierModule::init(ProtoMolApp *app) {
  ...
  InputHarmNM::registerConfiguration(&app->config, 0);
  InputOutputIntermediate::registerConfiguration(&app->config,0);
  InputOutputIntermediateStart::registerConfiguration(&app->config,0);
}

void StringModifierModule::postBuild(ProtoMolApp *app) {
...
void StringModifierModule::addModifiers(ProtoMolApp *app) {
  Modifier *modifier;
  ...
  int harmNM = app->config[InputHarmNM::keyword];
  if (harmNM) {
     modifier = new ModifierHarmNMRestForce(app->integrator);
     app->integrator->bottom()->adoptPreForceModifier(modifier);
     report << plain << "Set modifier which will give step() information to HarmNMRestForce"<<endr;
  }

  int output_intermediate = app->config[InputOutputIntermediate::keyword];
  int output_intermediate_start = app->config[InputOutputIntermediateStart::keyword];
  if (output_intermediate) {
     modifier = new ModifierOutputIntermediatePoints(app->integrator, output_intermediate_start);
     app->integrator->bottom()->adoptPostForceModifier(modifier);
     report << plain <<"Set modifier which will store intermediate points in PDB files once the target is reached."<<endr;
