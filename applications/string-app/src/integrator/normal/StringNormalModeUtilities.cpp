...
namespace ProtoMol {
  ...
  //constructors
  StringNormalModeUtilities::StringNormalModeUtilities(): firstMode(1), numMode(-1), myGamma(-1), mySeed(-1), myTemp(-1), mhQu(0)
    {/*tmpFX=NULL;*/ tmpC=NULL; invSqrtMass=NULL; sqrtMass=NULL;}

    StringNormalModeUtilities::StringNormalModeUtilities( int firstmode, int nummode, Real gamma, int seed, Real temperature):
        firstMode(firstmode), numMode(nummode), myGamma(gamma/ (1000 * Constant::INV_TIMEFACTOR)), mySeed(seed), myTemp(temperature)
  {
    /*tmpFX=NULL;*/ tmpC=NULL; invSqrtMass=NULL; sqrtMass=NULL;
  }

  StringNormalModeUtilities::~StringNormalModeUtilities() 
  {  
      //
  ...
  void StringNormalModeUtilities::initialize(int sz, GenericTopology *myTopo, Vector3DBlock * myForces, int nm_flags){
  ...
  bool StringNormalModeUtilities::testRemainingModes(){
    if((numMode + (firstMode-1)) < _3N) return true;
    else return false;
  }

  //Setup string of integrators so that all point to the top integrator 'Eigenpointers' :-)
  void StringNormalModeUtilities::setIntegratorSetPointers(Integrator *integrator, EigenvectorInfo *eipt, bool eiValid){

    StringNormalModeUtilities *nmint;	
    if(eiValid){
    ...
    for(Integrator* i = integrator->next();i != NULL;i = i->next()){
        nmint = dynamic_cast<StringNormalModeUtilities*>(i);
...
//   double* StringNormalModeUtilities::vector3DBlockTOvect(Vector3DBlock* blkDat, double* vecDat){
//       for( int i=0; i<3*_N; i++) vecDat[i] = (*blkDat)[i/3][i%3];
//       return vecDat;
//  }

  //Convert linear array from BLAS to Vector3DBlock 
//   Vector3DBlock* StringNormalModeUtilities::vectTOvector3DBlock(double* vecDat, Vector3DBlock* blkDat){
...
//   }
  ...
  Vector3DBlock* StringNormalModeUtilities::nonSubspaceForce(Vector3DBlock * force, Vector3DBlock * iPforce){
    //
  ...
  Vector3DBlock* StringNormalModeUtilities::nonSubspacePosition(Vector3DBlock * force, Vector3DBlock * iPforce){
    //
    if (force != iPforce)
      iPforce->intoAssign(*force);
    //vector3DBlockTOvect(force, tmpFX);	//put positions-x_0 into linear array
  ...
  Vector3DBlock* StringNormalModeUtilities::subspaceForce(Vector3DBlock * force, Vector3DBlock * iPforce){
    //
    if (force != iPforce)
      iPforce->intoAssign(*force);
    //vector3DBlockTOvect(force, tmpFX);	//put positions-x_0 into linear array
  ...
  Vector3DBlock* StringNormalModeUtilities::subspaceVelocity(Vector3DBlock * force, Vector3DBlock * iPforce){
    //
    if (force != iPforce)
      iPforce->intoAssign(*force);
    //vector3DBlockTOvect(force, tmpFX);	//put positions-x_0 into linear array
  ...
  void StringNormalModeUtilities::subSpaceSift(Vector3DBlock *velocities, Vector3DBlock *forces){
  ...
  void StringNormalModeUtilities::forceProjection(){
      if((*Q) != NULL){
  ...
  Vector3DBlock* StringNormalModeUtilities::cartSpaceProj(double *tmpC, Vector3DBlock * iPos, Vector3DBlock * ex0){
    //
#if defined(HAVE_LAPACK) || defined(HAVE_SIMTK_LAPACK)
  ...
  double* StringNormalModeUtilities::modeSpaceProj(double *cPos, Vector3DBlock * iPos, Vector3DBlock * ex0){
    //
  ...
  void StringNormalModeUtilities::genProjGauss(Vector3DBlock *gaussRandCoord, GenericTopology *myTopo) {
  ...
  void StringNormalModeUtilities::genProjGaussC(Vector3DBlock *gaussRandCoord, Vector3DBlock *gaussRandCoordm, GenericTopology *myTopo) {
    //generate set of random force variables and project into sub space
  ...
  void StringNormalModeUtilities::nmlDrift(Vector3DBlock *myPositions, Vector3DBlock *myVelocities, Real dt, GenericTopology *myTopo) {
  ...
  //Diagonalize hessian******************************************************************//
  int StringNormalModeUtilities::diagHessian(double *eigVecO, double *eigValO, double *hsnhessM, int dim, int &numFound){
   double *wrkSp;
   int *isuppz, *iwork;
  ...
  void StringNormalModeUtilities::absSort(double *eigVec, double *eigVal, int *eigIndx, int dim){
    int i;
  ...
  void StringNormalModeUtilities::getInnerHess(double *eigVec, double *hess, double *innerHess){
#if defined(HAVE_LAPACK) || defined(HAVE_SIMTK_LAPACK)
  ...
  void StringNormalModeUtilities::getNewEigs(double *eigVec, double *origEigVec, double *innerEigVec){
#if defined(HAVE_LAPACK) || defined(HAVE_SIMTK_LAPACK)
  ...
  double StringNormalModeUtilities::calcRayleigh(double *rQ, double *boundRq, double *hsnhessM, int numv, double raylAverage){
  ...
  int StringNormalModeUtilities::minimizer(Real peLim, int numloop, bool simpM, bool reDiag, bool nonSubspace, int *forceCalc, Real *lastLambda, 
    ...
    for(in=0;in<numloop;in++){
        ...
        //
        report.precision(10);
        report <<debug(6)<<"[StringNormalModeUtilities::minimizer] PE= "<<myEnergies->potentialEnergy()<<endr;
        ...
        report <<debug(7)<<"[StringNormalModeUtilities::minimizer] lambd= "<<lambda<<endl;
	  ...
	  if(rsCG>4) report << error << "[StringNormalModeUtilities::minimizer] Minimization failed, Aborting. "<<rsCG <<endr;
            else{
                if(!reDiag){  //allow minimization if mode at angle to sub-space
                    ...
                    *lastLambda -= lambda;
                    ...
                    rsCG++;
                    report <<debug(1)<<"[StringNormalModeUtilities::minimizer] Reset CG, PE fail. Cycle= "<<rsCG<<" lambda= "<<lambda<<endl;
                }else{
                    (*myPositions).intoWeightedAdd(-lambda,posTemp);		//reset positions
