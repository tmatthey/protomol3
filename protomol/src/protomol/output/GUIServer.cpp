...
GUIServer::GUIServer(const char *name, int natoms, int nbonds, int port, int prange) :
  request(GS_COORD_REQUEST), shutdown(0) {
...
#ifdef _WIN32
  if (thread) WaitForSingleObject(thread, 5000); // Upto 5sec
#endif
...
void GUIServer::serverThread() {
  ...
  if (bind(lsocket, (struct sockaddr *)&addr, sizeof(addr)) == SOCKET_ERROR) {
    ...
    if(com_port_range > 1){
        ...
        if(bret == SOCKET_ERROR){
            closesocket(lsocket);
            THROW(string("Could not bind to local socket: ") + strerror(errno));
        }
    }else{
        closesocket(lsocket);
        THROW(string("Could not bind to local socket: ") + strerror(errno));
  ...
  if (listen(lsocket, SOMAXCONN) == SOCKET_ERROR) {//
    closesocket(lsocket);
    THROW(string("Could not listen to local socket: ") + strerror(errno));
  }
  ...
  while (!shutdown){
    ...
    if(readsocks < 0)
        THROW(string("Socket non-blocking error: ") + strerror(errno));
    if(readsocks){
