  ...
  public:
    void evaluateBorn(TOneAtomPair &myOneAtomPair, GenericTopology *topo,
      ...
      for (unsigned int i = 0; i < topo->atoms.size(); i++) {
        ...
        Real f = topo->atoms[i].mySCPISM_A->sasaFrac;
        Real rw = topo->atoms[i].mySCPISM_A->R_w;
        Real A_i = topo->atomTypes[topo->atoms[i].type].mySCPISM_T->A_i;
        Real dR_vdw2 = topo->atoms[i].mySCPISM_A->dR_vdw2;
        Real gamma = 0.5; // Same for all
        //**********************************************
        // Eq. 3
        // Atoms that are not polar hydrogens have a 0
        // polarFrac, so this will reduce to Eq. 1
        topo->atoms[i].mySCPISM_A->bornRadius =
          rw + gamma - gamma * dR_vdw2 *
          (A_i + f) + topo->atoms[i].mySCPISM_A->polarFrac;
        //**********************************************
      }

      for (unsigned int i = 0; i < topo->atoms.size(); i++) {
        // More variable definitions
        Real bornRadius = topo->atoms[i].mySCPISM_A->bornRadius;
        Real rBornRadius = 1.0 / topo->atoms[i].mySCPISM_A->bornRadius;
        Real K = (D - 1.0) / 2;
        Real alpha_i = topo->atomTypes[topo->atoms[i].type].mySCPISM_T->alpha;
        ...
        for (int k = 0;
          ...
          // J term
          Real bornRadius_j = topo->atoms[j].mySCPISM_A->bornRadius;
          Real rBornRadius_j = 1.0 / bornRadius_j;
          Real alpha_j = topo->atomTypes[topo->atoms[j].type].mySCPISM_T->alpha;
          Real Ds_r_j =
      ...
      for (unsigned int b = 0; b < topo->atoms.size(); b++) {
        topo->atoms[b].mySCPISM_A->sasaFrac = 0.0;
        topo->atoms[b].mySCPISM_A->polarFrac = 0.0;
      }
