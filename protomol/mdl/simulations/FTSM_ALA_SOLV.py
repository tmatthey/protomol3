...
for i in range(0, numpoints):
    ...
    for j in range(0, 2):
        io.readPDBPos(physarray[i][j], "data/alanDipeptideSol/alan_wb10_mineq.pdb")
        io.readPSF(physarray[i][j], "data/alanDipeptideSol/alan_wb10.psf")
        io.readPAR(physarray[i][j], "data/alanDipeptideSol/par_all27_prot_lipid.inp")
        physarray[i][j].bc = "Periodic"
...
#ff.constrainDihedral(PHI_DIHEDRAL, PSI_DIHEDRAL)
...
#ff.setCutoff("LennardJones", 20);
#ff.setParameters("LennardJones", "switchon=15.0")
#ff.setSwitching("CoulombDiElec", "C2")
#ff.setAlgorithm("CoulombDiElec", "Cutoff")
#ff.setCutoff("CoulombDiElec", 20);
#ff.setParameters("CoulombDiElec", "switchon=15.0")
...
for iter in range(0, 200000): # NUMBER OF FTSM ITERATIONS
    for workpt in range(0, numpoints): # LOOPING OVER POINTS
        if (iter >= 10000 and iter <= 100000):
            kappa += (100.-float(sys.argv[1]))/90000.
        # UPDATE FREE SPACE
        ...
        #proparray[workpt][0].propagate(scheme="velocityscale", steps=1, dt=dt, forcefield=ff, params={"gamma":0.1})

#        proparray[workpt][1].propagate(scheme="velocityscale", steps=1, dt=dt, forcefield=ff)

        proparray[workpt][0].propagate(scheme="LangevinImpulse", steps=1, dt=dt, forcefield=ff, params={"gamma":0.1})

        proparray[workpt][1].propagate(scheme="LangevinImpulse", steps=1, dt=dt, forcefield=ff, params={"gamma":0.1})
